...
class QueueDb:
    def __init__(self, db_name, db_type, query_list, data_func, db_func=None, sync_num=10):
        ...
        self.list_num = 30
        if len(self.query_list) < self.list_num * self.sync_num:
            self.sync_num = 1
    ...
    async def client_list(self, client, client_index, frag_list):
        for loop_time in range(math.ceil(len(self.query_list)/(self.list_num * self.sync_num))):
            query_index = [j + self.list_num * self.sync_num * loop_time for j in frag_list]
            data_return = {}
            for index in query_index:
                ...
                for retry in range(4):
                    ...
                    data_status = self.data_func(query_data)
                    if data_status:
                        ...
                        time.sleep(5)
                        await client.login()
            if self.db_func and data_return:
                self.db_func(self.db_folder+str(client_index)+'-'+self.db_name, data_return)

    async def queue_main(self):
        tasks = []
        for client_id in range(self.sync_num):
            client = await create_client(client_id)
            frag_list = list(range(client_id*self.list_num, self.list_num+client_id*self.list_num))
